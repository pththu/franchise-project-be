...
public class CustomerServiceImpl implements CustomerService {
        ...
        @Override
        public PageResponse<CustomerFranchiseResponse> getCustomersForManager(UUID franchiseId, CustomerStatus status,
                        Pageable pageable) {
                Page<CustomerFranchise> page = (status == null)
        ...
        public List<CustomerFranchiseResponse> getCustomersByIds(List<UUID> ids) {
                ...
                return franchises.stream().map(cf -> {
                        CustomerFranchiseResponse response = customerFranchiseMapper.toCustomerFranchiseResponse(cf,
                                        identityClient, franchiseClient);
        ...
        @Override
        public PageResponse<CustomerFranchiseResponse> searchCustomers(UUID franchiseId, CustomerStatus status,
        ...
        public Page<CustomerSummaryResponse> searchCustomers(SearchRequest request) {
                ...
                List<CustomerSummaryResponse> assembled = allCustomer.stream()
                                .map(user -> {
                                        ...
                                        List<CustomerFranchiseSummary> franchiseSummaries = userCFs.stream()
        ...
        public CustomerFranchise saveCustomerFranchise(SaveCustomerRequest request) {
                // boolean isExisted =
                // customerFranchiseRepository.existsByUserIdAndFranchiseId(userId,
                // franchiseId);
        ...
        public CustomerFranchiseResponse getCustomerOfFranchiseById(UUID userId, UUID franchiseId) {
                ...
                CustomerFranchise customerFranchise = customerFranchiseRepository
                                .findByUserIdAndFranchiseId(userId, franchiseId)
                                .orElseThrow(() -> new AppException(ErrorCode.CUSTOMER_NOT_FOUND));
        ...
        // @Override
        // @Transactional
        // public CustomerFranchise createCustomerAtFranchise(UUID userId, UUID
        // franchiseId, CustomerType type) {
        // if (customerFranchiseRepository.existsByUserIdAndFranchiseId(userId,
        // franchiseId)) {
        ...
        private PageResponse<CustomerFranchiseResponse> buildPageResponse(Page<CustomerFranchise> pageResult) {
                ...
                Map<UUID, CustomerTierResponse> loyaltyMap = loyaltyClient.getBulkCustomerTierInfo(userIds).stream()
                                ...
                                .collect(Collectors.toMap(CustomerTierResponse::getUserId, tier -> tier,
                                                (t1, t2) -> t1));

                List<CustomerFranchiseResponse> responses = pageResult.getContent().stream()
                                .map(cf -> {
                                        CustomerFranchiseResponse response = customerFranchiseMapper
                                                        .toCustomerFranchiseResponse(cf, identityClient,
                                                                        franchiseClient);
        ...
        private boolean matchesFilter(CustomerSummaryResponse summary, SearchRequest request) {
                ...
                if (StringUtils.hasText(request.getKeyword())) {
                        ...
                        boolean matchUser = (user.getUsername() != null
